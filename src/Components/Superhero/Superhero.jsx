...
export default function Mmorpg() {
  ...
  async function getProduct() {
    ...
    try {
      ...
      // Use reduce() to filter "Shooter" games, sort them, and slice the first 8
      const sortedGames = data.reduce((acc, game) => {
        // Filter only games that include "Shooter" in the genre
        if (game.genre.includes("Shooter")) {
          acc.push(game);
        }
        return acc;
      }, [])
      .sort((a, b) => a.title.localeCompare(b.title)) // Sort alphabetically by title
      .slice(0, 8); // Slice the first 8 games
